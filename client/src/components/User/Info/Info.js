...
class Info extends Component {
  ...
  componentDidMount() {
    ...
    const loadUntilScrollable = () => {
      ...
      if ((document.getElementsByClassName('component-wrapper')[0].offsetHeight) < window.innerHeight) {
    ...
    document.getElementsByClassName('app-container')[0].addEventListener('scroll', () => {
      // WHEN HEIGHT UNDER SCROLLBOTTOM IS LESS THEN 250

      // if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight)
      if ((document.getElementsByClassName('app-container')[0].scrollTop) + 100 >= (document.getElementsByClassName('component-wrapper')[0].offsetHeight - window.innerHeight)) {
