import { AuthStrategies } from '~lib/types';
import { UserModel } from '../api/user.model';
import { TokenModel } from '../api/token/model';

class TestUtils {
  public get localTestUsers(): UserModel[] {
    return this.users.filter((user) => {
      return user.strategy === AuthStrategies.local;
    });
  }

  public get socialTestUser(): UserModel[] {
    return this.users.filter((user) => {
      return user.strategy !== AuthStrategies.local;
    });
  }

  public static resCookieParser(cookieString: string[]): {
    [key: string]: string,
  } {
    if (!cookieString || cookieString.length < 1) {
      return {};
    }

    return cookieString
      .reduce(
        (acc, curr) => {
          const payload = curr.split(';')[0];
          const cookiePair = payload && payload.split('=');

          return {
            ...acc,
            ...(payload && cookiePair && {
              [cookiePair[0]]: cookiePair[1],
            }),
          };
        },
        {},
      );
  }

  public imgData: string;
  public mockedToken: string;
  public newUsers: UserModel[];
  public users: UserModel[];

  constructor() {
    // tslint:disable-next-line:max-line-length
    this.imgData = 'data:image/png;base64,';
    this.mockedToken = 'kfjaahsdlkfjhavlasdjfbnalsdfa';
    this.newUsers = [
      {
        display_name: 'newuser',
        email: 'newuser@damso.com',
        id: null,
        password: 'newpassword',
      },
      {
        display_name: 'urlavataruser',
        email: 'urlavataruser@damso.com',
        id: null,
        password: 'urlavatarpassword',
      },
      {
        display_name: 'dataavataruser',
        email: 'dataavataruser@damso.com',
        id: null,
        password: 'dataavatarpassword',
      },
    ] as UserModel[];
    this.users = [
      {
        avatar: null,
        display_name: 'tester_one',
        email: 'tester_one@damso.com',
        id: '138edb05-d729-4cc0-a6a7-b593564380f0',
        password: 'tester_one',
        strategy: AuthStrategies.local,
      },
      {
        avatar: null,
        display_name: 'tester_two',
        email: 'tester_two@damso.com',
        id: '3be019e1-2cea-48c2-aaea-c0dad172b2af',
        password: 'tester_two',
        strategy: AuthStrategies.local,
      },
      {
        avatar: null,
        display_name: 'tester_three',
        email: 'tester_three@damso.com',
        id: 'a2221031-7d99-45a5-b801-1860e876edda',
        password: 'tester_three',
        strategy: AuthStrategies.local,
      },
      {
        avatar: null,
        display_name: 'social_tester_one',
        id: 'fe9b005a-f9fa-451f-8606-bbeb1c3178f2',
        social_id: 'ec2cd494d3c8484aaaa36e7663e42c17',
        strategy: 'google',
      },
    ] as UserModel[];
  }

  public async emptyTables() {
    await TokenModel.emptyTable();
    await UserModel.emptyTable();
  }

  public async insertNewUsers(users: UserModel[] = this.users) {
    this.users = await UserModel.registerNewUsers(users);
  }
}

export const testUtils = new TestUtils();

export const mockedToken = 'kfjaahsdlkfjhavlasdjfbnalsdfa';

export const newUsers = [
  {
    display_name: 'newuser',
    email: 'newuser@damso.com',
    id: null,
    password: 'newpassword',
  },
  {
    display_name: 'urlavataruser',
    email: 'urlavataruser@damso.com',
    id: null,
    password: 'urlavatarpassword',
  },
  {
    display_name: 'dataavataruser',
    email: 'dataavataruser@damso.com',
    id: null,
    password: 'dataavatarpassword',
  },
];

// tslint:disable-next-line:max-line-length
export const imgData = 'data:image/png;base64,';

export const resCookieParser = (cookieString: string[]): {
  [key: string]: string,
} => {
  if (!cookieString || cookieString.length < 1) {
    return {};
  }

  return cookieString
    .reduce(
      (acc, curr) => {
        const payload = curr.split(';')[0];
        const cookiePair = payload && payload.split('=');

        return {
          ...acc,
          ...(payload && cookiePair && {
            [cookiePair[0]]: cookiePair[1],
          }),
        };
      },
      {},
    );
};

export const testUsers = [
  {
    avatar: null,
    display_name: 'tester_one',
    email: 'tester_one@damso.com',
    id: '138edb05-d729-4cc0-a6a7-b593564380f0',
    password: 'tester_one',
    strategy: 'local',
  },
  {
    avatar: null,
    display_name: 'tester_two',
    email: 'tester_two@damso.com',
    id: '3be019e1-2cea-48c2-aaea-c0dad172b2af',
    password: 'tester_two',
    strategy: 'local',
  },
  {
    avatar: null,
    display_name: 'tester_three',
    email: 'tester_three@damso.com',
    id: 'a2221031-7d99-45a5-b801-1860e876edda',
    password: 'tester_three',
    strategy: 'local',
  },
  {
    avatar: null,
    display_name: 'social_tester_one',
    id: 'fe9b005a-f9fa-451f-8606-bbeb1c3178f2',
    social_id: 'ec2cd494d3c8484aaaa36e7663e42c17',
    strategy: 'google',
  },
];
